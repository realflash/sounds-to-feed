    ...
    async def _download_episode(self, pid: str, prog_name: str, episode: str):
        ...
        try:
            ...
            if expected_file.exists():
                file_to_check = expected_file
            else:
                matching_files = list(output_dir.glob(f"{file_prefix}*"))
                if matching_files:
                    file_to_check = matching_files[0]

            if file_to_check:
                self.state_manager.mark_downloaded(pid, str(file_to_check))
                logger.info(f"Successfully downloaded {pid} to {file_to_check}")

                # Extract and save cover art sidecar
                try:
                    from mutagen.mp4 import MP4

                    audio = MP4(str(file_to_check))
                    if "covr" in audio.tags and audio.tags["covr"]:
                        covr = audio.tags["covr"][0]
                        cover_file = file_to_check.with_suffix(".jpg")
                        cover_file.write_bytes(bytes(covr))
                        logger.info(f"Extracted and saved sidecar cover to {cover_file}")
                except Exception as e:
                    logger.error(f"Failed to extract cover art during download of {pid}: {e}")
            else:
